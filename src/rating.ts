...
function getResults(): GameResult[] {
  ...
  for (const raw_result of raw_results) {
    const winner: Player = new Player(
      raw_result.winner.id,
      raw_result.winner.name,
      raw_result.winner.grade
    );
    const loser: Player = new Player(
      raw_result.loser.id,
      raw_result.loser.name,
      raw_result.loser.grade
    );
    results.push(
      new GameResult(
        raw_result.month,
